...
class MessageCleanupPlugin {
    constructor(app, client, ensureAuthenticated, hasAdminPermissions) {
        ...
        this.MODERATOR_ROLE_ID = '957213892810010645';
        ...
        return {
            ...
            html: `<div class="plugin-container">
                <div class="plugin-header">
                    ...
                    <div class="security-notice" style="background: rgba(255, 193, 7, 0.1); border: 1px solid rgba(255, 193, 7, 0.3); border-radius: 8px; padding: 12px; margin-top: 10px;">
                        <strong>⚠️ Security Notice:</strong> This tool requires Administrator or Moderator permissions. All actions are logged and monitored.
                    </div>
                </div>
                ...
                <div class="search-section" style="background: rgba(100, 149, 237, 0.1); border-radius: 10px; padding: 20px; margin-bottom: 20px;">
                    ...
                    <div class="search-form" style="display: grid; grid-template-columns: 1fr 1fr auto; gap: 10px; align-items: end;">
                        <div>
                            ...
                            <small style="color: rgba(255,255,255,0.7); font-size: 0.8em;">Discord user IDs only</small>
                        </div>
                        <div>
            ...
            script: `
                ...
                // Load channels and logs on page load
                ...
                async function searchUserMessages() {
                    ...
                    // Enhanced validation for Discord user ID
                    const userId = userInput.replace(/[<@!>]/g, '');
                    
                    if (!/^\\d{17,19}$/.test(userId)) {
                        showNotification('Invalid user ID format. Please enter a valid Discord user ID (17-19 digits)', 'error');
                        return;
                    }
                    ...
                    try {
                        ...
                        const url = '/api/plugins/cleanup/messages/' + currentServerId + '/' + userId + 
                ...
                function displayUserInfo(user) {
                    ...
                    userInfoSection.innerHTML = \`
                        <div class="user-card" style="display: flex; align-items: center; gap: 15px; background: rgba(255,255,255,0.05); padding: 15px; border-radius: 10px;">
                            <img src="\${user.avatar}" alt="Avatar" style="width: 50px; height: 50px; border-radius: 50%;" onerror="this.src='data:image/svg+xml;base64,'">
                            <div>
                ...
                function displayMessages(messages) {
                    ...
                    container.innerHTML = messages.map(msg => {
                        ...
                        const ageWarning = isOld ? '<div style="color: #ffa502; font-size: 0.8em; margin-top: 5px;">⚠️ Old message (>14 days) - will delete individually</div>' : '';
                        ...
                        <div class="message-item" data-message-id="\${msg.id}" style="background: rgba(255,255,255,0.05); border-radius: 8px; padding: 15px; margin-bottom: 10px; border-left: 3px solid \${isOld ? '#ffa502' : '#7289da'};">
                            <div class="message-header" style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 10px;">
                                <div style="display: flex; align-items: center; gap: 10px;">
                                    ...
                                    <span class="channel-name" style="background: rgba(114, 137, 218, 0.2); color: #7289da; padding: 2px 8px; border-radius: 12px; font-size: 0.8em;">#\${msg.channelName}</span>
                                    <span class="timestamp" style="font-size: 0.8em; opacity: 0.7;">\${new Date(msg.timestamp).toLocaleString()}</span>
                                </div>
                            ...
                            <div class="message-content" style="background: rgba(0,0,0,0.2); padding: 10px; border-radius: 5px;">
                                <p style="margin: 0; word-break: break-word; max-height: 100px; overflow-y: auto;">\${msg.content}</p>
                                \${msg.hasMedia ? '<div style="margin-top: 5px; font-size: 0.8em; opacity: 0.8;">📎 Has attachments/embeds</div>' : ''}
                                \${ageWarning}
                            </div>
                        </div>
                        \`;
                ...
                function deleteSelectedMessages() {
                    ...
                    document.getElementById('deleteCount').textContent = selectedMessages.size;
                ...
                async function confirmDeletion() {
                    ...
                    if (reason.length > 500) {
                        showNotification('Reason too long (max 500 characters)', 'error');
                        return;
                    }
                    ...
                    try {
                        ...
                        let message = \`Deleted \${result.success} messages\`;
                        if (result.failed > 0) {
                ...
                async function deleteMessage(messageId) {
                    ...
                    try {
                        ...
                        if (result.success > 0) {
                            ...
                            showNotification('Failed to delete message: ' + (result.errors[0] || 'Unknown error'), 'error');
                        }
                ...
                async function loadCleanupLogs() {
                    try {
                        const response = await fetch('/api/plugins/cleanup/logs/' + currentServerId + '?limit=50');
                        ...
                        container.innerHTML = logs.map(log => \`
                            <div class="log-item" style="background: rgba(255,255,255,0.03); padding: 12px; border-radius: 8px; margin-bottom: 10px; border-left: 3px solid #7289da;">
                                <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 8px;">
                                    <span style="font-weight: 500;">\${new Date(log.timestamp).toLocaleString()}</span>
                                    <span class="method-badge" style="background: rgba(114, 137, 218, 0.3); padding: 3px 10px; border-radius: 12px; font-size: 0.8em; font-weight: 500;">
                                        \${log.method === 'slash_command_bulk' ? '🔧 Bulk Command' : 
                                          log.method === 'slash_command_individual' ? '🔧 Individual Command' : 
                                          log.method.includes('slash_command') ? '🔧 Command' : '🖥️ Dashboard'}
                                    </span>
                                </div>
                                <div style="opacity: 0.8; font-size: 0.9em; margin-bottom: 5px;">
                                    <strong>Channel:</strong> <#\${log.channelId}> | 
                                    <strong>Target:</strong> <@\${log.targetUserId}> | 
                                    <strong>Moderator:</strong> <@\${log.moderatorId}>
                                </div>
                                \${log.reason ? \`<div style="font-style: italic; opacity: 0.7; font-size: 0.85em; background: rgba(0,0,0,0.2); padding: 5px 8px; border-radius: 4px;">Reason: \${log.reason}</div>\` : ''}
                            </div>
                        \`).join('');
                ...
                // Real-time validation for user search
                document.getElementById('userSearch').addEventListener('input', (e) => {
                    ...
                    if (value && !/^<?@?!?\\d{17,19}>?$/.test(value)) {
                        ...
                        e.target.title = 'Invalid Discord user ID format';
                    } else {
                ...
                function showNotification(message, type = 'info') {
                    ...
                    notification.style.cssText = \`
                        ...
                        background: \${type === 'error' ? 'linear-gradient(135deg, #ff4757, #ff3742)' : 
                                   type === 'success' ? 'linear-gradient(135deg, #2ed573, #1e90ff)' : 
                                   type === 'warning' ? 'linear-gradient(135deg, #ffa502, #ff6348)' : 
                                   'linear-gradient(135deg, #5352ed, #7289da)'};
                    \`;
                ...
                // Add CSS animations
                if (!document.querySelector('#cleanup-animations')) {
                    const style = document.createElement('style');
                    style.id = 'cleanup-animations';
                    style.textContent = \`
