    ...
    setupRoutes() {
        ...
        this.app.get('/api/plugins/cleanup/messages/:serverId/:userId', this.ensureAuthenticated, async (req, res) => {
            try {
                ...
                try {
					console.log('Debug: Checking admin permissions for user:', req.user.id, 'server:', serverId);
					const hasAdmin = await this.hasAdminPermissions(req.user.id, serverId);
					console.log('Debug: hasAdminPermissions result:', hasAdmin);
					
					if (!hasAdmin) {
						console.log('Debug: User does not have admin permissions');
						return res.status(403).json({ error: 'Admin permissions required' });
					}
					console.log('Debug: Admin permissions check passed');
				} catch (error) {
					console.error('Debug: Error checking admin permissions:', error);
                ...
                (function() {
                    ...
                    async function searchUserMessages() {
                        ...
                        try {
                            ...
                            const url = \`/api/plugins/cleanup/messages/\${currentServerId}/\${userId}\${channelId ? '?channelId=' + channelId : ''}\`;
