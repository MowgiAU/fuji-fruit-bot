...
class AutoRolePlugin {
	...
	setupRoutes() {
        ...
        // --- FIX: Corrected the reaction role creation logic ---
        this.app.post('/api/plugins/autorole/reactionroles/:serverId/create', this.ensureAuthenticated, async (req, res) => {
            try {
                ...
                for (const roleData of roles) {
                    ...
                    if (role) {
                        const userInputEmoji = roleData.emoji.trim();
                        let resolvedEmojiIdentifier;
                        let displayEmoji;

                        // Try to find the custom emoji on the server
                        const customEmoji = guild.emojis.cache.find(e => 
                            e.toString() === userInputEmoji || 
                            e.name === userInputEmoji.replace(/:/g, '')
                        );

                        if (customEmoji) {
                            // It's a valid custom emoji from this server
                            resolvedEmojiIdentifier = customEmoji.toString();
                            displayEmoji = customEmoji.toString();
                        } else {
                            // Assume it's a standard Unicode emoji
                            resolvedEmojiIdentifier = userInputEmoji;
                            displayEmoji = userInputEmoji;
                        }

                        embed.fields.push({
                            name: displayEmoji, // Use the display-friendly version
                            value: `${role.name}${roleData.description ? '\n' + roleData.description : ''}`,
                            inline: true
                        });
                        
                        // Store the mapping with the resolved identifier as the key
                        emojiMap[resolvedEmojiIdentifier] = roleData.roleId;
                        reactionsToAdd.push(resolvedEmojiIdentifier);
                    }
                }
                
                const message = await channel.send({ embeds: [embed] });
                
                // Add reactions using the resolved identifiers
    ...
    async handleReactionAdd(reaction, user) {
        try {
            ...
            const roleId = reactionRoleData.roles[emojiIdentifier];
            ...
            if (!role) {
                console.log(`[Reaction Roles] Role not found: ${roleId}`);
                return;
            }
            ...
            if (reactionRoleData.maxRoles > 0) {
                ...
                const memberReactionRoles = member.roles.cache.filter(r => 
                    currentReactionRoles.includes(r.id)
                );
            ...
            try {
                ...
                await this.logAutoRole(reaction.message.guild, {
                    ...
                    user: user,
                    roles: [role],
                    reason: `Reacted with ${emojiIdentifier}`,
                    messageId: messageId
                });
                ...
                if (error.code === 50013) {
                     console.error('[Reaction Roles] Missing permissions to manage roles. Make sure the bot role is above the roles it needs to assign.');
                }
            }
        ...
        try {
            ...
            try {
                ...
                await this.logAutoRole(reaction.message.guild, {
                    ...
                    user: user,
                    roles: [role],
                    reason: `Removed reaction ${emojiIdentifier}`,
                    messageId: messageId
                });
    ...
    async logAutoRole(guild, data) {
        try {
            ...
            if (eventType.startsWith('join')) {
                logChannelId = this.autoRoleSettings[guildId]?.logChannelId;
            } else if (eventType.startsWith('level')) {
                logChannelId = this.levelRoles[guildId]?.logChannelId;
            } else if (eventType.startsWith('reaction')) {
                logChannelId = this.autoRoleSettings[guildId]?.logChannelId;
            }
            ...
            html: `
                ...
                <div id="level-role-modal" style="display: none; position: fixed; top: 0; left: 0; width: 100%; height: 100%; background: rgba(0,0,0,0.7); z-index: 1000; justify-content: center; align-items: center;">
                    <div style="background: linear-gradient(135deg, #1a1a2e 0%, #16213e 50%, #0f3460 100%); border-radius: 15px; padding: 2rem; max-width: 500px; width: 90%; border: 1px solid rgba(255,255,255,0.2);">
                        ...
                        <div class="form-group"><label for="level-role-level">Level:</label><input type="number" id="level-role-level" class="form-control" min="1" max="999" placeholder="Level required"></div>
                        <div class="form-group"><label for="level-role-role">Role:</label><select id="level-role-role" class="form-control"><option value="">Select a role...</option></select></div>
                        <div class="form-group"><label><input type="checkbox" id="level-role-remove-old"> Remove Lower Level Roles</label><small style="opacity: 0.7; display: block; margin-top: 4px;">Automatically remove roles from lower levels when this role is assigned</small></div>
                        <div style="display: flex; gap: 10px; margin-top: 1.5rem;"><button type="button" id="save-level-role" class="btn-primary">Add Role</button><button type="button" id="cancel-level-role" class="glass-btn">Cancel</button></div>
                    </div>
                </div>
                <div id="reaction-role-modal" style="display: none; position: fixed; top: 0; left: 0; width: 100%; height: 100%; background: rgba(0,0,0,0.7); z-index: 1000; justify-content: center; align-items: center; overflow-y: auto;">
                    <div style="background: linear-gradient(135deg, #1a1a2e 0%, #16213e 50%, #0f3460 100%); border-radius: 15px; padding: 2rem; max-width: 700px; width: 90%; max-height: 90vh; overflow-y: auto; border: 1px solid rgba(255,255,255,0.2); margin: 20px;">
                        ...
                        <div class="form-group"><label for="rr-channel">Channel:</label><select id="rr-channel" class="form-control"><option value="">Select a channel...</option></select></div>
                        <div class="form-group"><label for="rr-title">Title:</label><input type="text" id="rr-title" class="form-control" placeholder="Role Selection" maxlength="256"></div>
                        <div class="form-group"><label for="rr-description">Description:</label><textarea id="rr-description" class="form-control" rows="3" placeholder="React to get roles!" maxlength="2048"></textarea></div>
                        <div class="form-group"><label for="rr-max-roles">Maximum Roles (0 = unlimited):</label><input type="number" id="rr-max-roles" class="form-control" min="0" max="20" value="0"></div>
                        <div class="form-group"><label><input type="checkbox" id="rr-remove-on-unreact" checked> Remove Role When Unreacted</label></div>
                        <div class="form-group"><label>Roles:</label><div id="rr-roles-list" style="min-height: 100px; background: rgba(255,255,255,0.05); border-radius: 8px; padding: 10px; margin-bottom: 10px;"><div id="no-rr-roles" style="opacity: 0.6; text-align: center; padding: 20px;">No roles added yet</div></div><div style="display: flex; gap: 10px; margin-bottom: 10px;"><input type="text" id="rr-emoji" class="form-control" placeholder="Emoji" style="max-width: 80px;"><select id="rr-role" class="form-control" style="flex: 1;"><option value="">Select role...</option></select><button type="button" id="add-rr-role" class="glass-btn">Add</button></div><div class="form-group"><label for="rr-role-description">Role Description (optional):</label><input type="text" id="rr-role-description" class="form-control" placeholder="Additional description for this role" maxlength="100"></div></div>
                        <div style="display: flex; gap: 10px; margin-top: 1.5rem;"><button type="button" id="create-rr-message" class="btn-primary">Create Message</button><button type="button" id="cancel-rr-message" class="glass-btn">Cancel</button></div>
                    </div>
                </div>
            `,
            script: `
                // Auto-Role Plugin Frontend Logic (script remains unchanged)
                (function() {
                    ...
                    function displayLevelRoles() {
                        ...
                        list.innerHTML = '';
                        levelRoles.sort((a, b) => a.level - b.level).forEach((levelRole, index) => {
                            ...
                            roleElement.innerHTML = \`<div><div style="font-weight: 600; margin-bottom: 4px;">Level \${levelRole.level}</div><div style="font-size: 0.9rem; opacity: 0.8;">\${role ? role.name : 'Unknown Role'}</div><div style="font-size: 0.8rem; opacity: 0.6; margin-top: 2px;">\${levelRole.removeOldRoles ? 'Removes lower level roles' : 'Keeps lower level roles'}</div></div><button type="button" onclick="window.removeLevelRole(\${index})" class="glass-btn-small">Remove</button>\`;
                    ...
                    function displayReactionRoles(reactionRoles) {
                        ...
                        messages.forEach(message => {
                            ...
                            messageElement.innerHTML = \`<div><div style="font-weight: 600; margin-bottom: 4px;">\${message.title || 'Reaction Role Message'}</div><div style="font-size: 0.9rem; opacity: 0.8;">\${roleCount} role(s) • Created \${createdAt}</div><div style="font-size: 0.8rem; opacity: 0.6; margin-top: 2px;">Max roles: \${message.maxRoles || 'Unlimited'} • Remove on unreact: \${message.removeOnUnreact ? 'Yes' : 'No'}</div></div><button type="button" onclick="window.deleteReactionRole('\${message.messageId}')" class="glass-btn-small">Delete</button>\`;
                    ...
                    function displayReactionRoleRoles() {
                        ...
                        reactionRoleRoles.forEach((roleData, index) => {
                            ...
                            roleElement.innerHTML = \`<div style="display: flex; align-items: center; gap: 8px;"><span style="font-size: 1.2em;">\${roleData.emoji}</span><div><div style="font-weight: 600;">\${roleData.roleName}</div>\${roleData.description ? \`<div style="font-size: 0.8rem; opacity: 0.7;">\${roleData.description}</div>\` : ''}</div></div><button type="button" onclick="window.removeReactionRoleRole(\${index})" class="glass-btn-small">Remove</button>\`;
                            list.appendChild(roleElement);
                        });
                    }
                    ...
                    async function saveAutoRoleSettings() {
                        ...
                        try {
                            ...
                            const joinRolesMinAge = document.getElementById('join-roles-min-age');
                            const logChannelSelect = document.getElementById('log-channel-select');
                            const selectedJoinRoles = joinRolesSelect ? Array.from(joinRolesSelect.selectedOptions).map(option => option.value) : [];
