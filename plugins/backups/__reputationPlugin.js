...
class ReputationPlugin {
    ...
    setupRoutes() {
    // Repair reputation data (admin only)
    ...
    async handleThanksMessage(message, settings) {
        ...
        if (message.reference?.messageId) {
            try {
                ...
                // Ignore if replied message is deleted
            }
        }
    ...
    async handleReasonModal(interaction) {
		...
		const amount = 1; // Always give 1 rep from modals to prevent abuse
    ...
    async adjustReputation(guildId, userId, fromUserId, category, amount, reason, type) {
        ...
        // Merge the existing data with the default to guarantee all keys exist
        data.users[userId][guildId] = {
            ...defaultRep,
            ...(data.users[userId][guildId] || {}), // Load existing data
            categories: {
                ...defaultRep.categories, // Start with all default categories (like legacy: 0)
                ...((data.users[userId][guildId]?.categories) || {}) // Overwrite with existing category values
            },
            streaks: {
                ...defaultRep.streaks,
                ...((data.users[userId][guildId]?.streaks) || {})
            }
        };
    ...
    // Add this temporary debug function to your reputationPlugin.js to check the data
async debugUserReputation(interaction, userId, guildId) {
    const data = await this.loadData();
    const rawUserData = data.users[userId]?.[guildId];
    
    console.log('=== DEBUG: Raw user data ===');
    console.log('User ID:', userId);
    console.log('Guild ID:', guildId);
    console.log('Raw data:', JSON.stringify(rawUserData, null, 2));
    ...
    const processedData = getUserData(rawUserData);
    console.log('=== DEBUG: Processed data ===');
    console.log('Processed data:', JSON.stringify(processedData, null, 2));
    
    console.log('=== DEBUG: Category breakdown ===');
    Object.entries(processedData.categories).forEach(([cat, val]) => {
        console.log(`${cat}: ${val}`);
    });
    
    console.log('=== DEBUG: Total calculation ===');
    const calculatedTotal = Object.values(processedData.categories).reduce((sum, val) => sum + (val || 0), 0);
    console.log('Calculated total from categories:', calculatedTotal);
    console.log('Stored total:', processedData.total);
    console.log('Difference:', processedData.total - calculatedTotal);
    
    return processedData;
}

	// Temporarily modify your handleRepView function to include debug logging
	async handleRepView(interaction) {
		const targetUser = interaction.options.getUser('user') || interaction.user;
		const data = await this.loadData();
		
		// ADD THIS DEBUG LINE
		const debugData = await this.debugUserReputation(interaction, targetUser.id, interaction.guild.id);
		...
		// Add legacy reputation separately if it exists
		const legacyRep = userRep.categories.legacy || 0;
		console.log('=== DEBUG: Legacy check ===');
		console.log('Legacy reputation value:', legacyRep);
		console.log('Legacy > 0?', legacyRep > 0);
		
		if (legacyRep > 0) {
			embed.addFields({ 
				name: `${this.getCategoryEmoji('legacy')} Migrated Reputation`, 
				value: `From a previous system: **${legacyRep}**`, 
				inline: false 
			});
		} else {
			// ADD THIS TEMPORARY DEBUG FIELD
			embed.addFields({ 
				name: '🔍 Debug Info', 
				value: `Legacy: ${legacyRep}, Total: ${userRep.total}, Categories total: ${Object.values(userRep.categories).reduce((s,v) => s+(v||0), 0)}`, 
				inline: false 
			});
		}
    ...
    async handleRepLeaderboard(interaction) {
        ...
        const embed = new EmbedBuilder()
            ...
            .setDescription(leaderboard.map((entry, index) => {
                ...
                let amount;
                if (type === 'weekly') {
                    amount = entry.weeklyRep || 0;
                } else if (type === 'monthly') {
                    amount = entry.monthlyRep || 0;
                } else if (type === 'overall') {
                    amount = entry.total || 0;
                } else {
                    amount = entry.categories[type] || 0;
                }
                ...
                <div class="plugin-container">
                    ...
                    <div id="repAdminSection" style="display: none;" class="settings-section">
                        ...
                        <div style="display: grid; gap: 1rem; grid-template-columns: repeat(auto-fit, minmax(300px, 1fr));">
							...
							<div class="admin-card">
								<h4>Data Repair</h4>
								<p>Fix reputation totals that don't match category sums by moving missing reputation to legacy category.</p>
                ...
                (function() {
					...
					if (document.getElementById('repairDataBtn')) {
						document.getElementById('repairDataBtn').addEventListener('click', async () => {
							...
							if (!confirm('This will move missing reputation to the legacy category. Continue?')) return;
    ...
    // ... (rest of the helper methods are correct) ...
