        ...
        return {
            ...
            script: `
				...
				(function() {
					...
					// Show backup section when a server is selected
					// Update your existing server selection logic to include:
					if (levelingServerSelect) {
						levelingServerSelect.addEventListener('change', function() {
							currentGuildId = this.value;
							if (currentGuildId) {
					...
					window.restoreBackup = async function(filename) {
						if (!confirm(\`Are you sure you want to restore from backup "\${filename}"? This will overwrite current data. A backup of the current state will be created first.\`)) {
							return;
						}
						...
						try {
							...
							if (response.ok) {
								if (window.showNotification) {
									window.showNotification(\`Successfully restored from \${filename}!\`, 'success');
								}
								loadBackups(); // Refresh the list
