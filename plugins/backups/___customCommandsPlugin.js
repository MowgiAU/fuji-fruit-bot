...
class CustomCommandsPlugin {
    ...
    // --- FIX START: Corrected reaction handling logic ---
    async handleReactionTriggers(reaction, user, action) {
        try {
            // Handle partial reactions (when the bot starts after the message was sent)
            if (reaction.partial) {
                try {
                    await reaction.fetch();
                } catch (error) {
                    console.error('Failed to fetch reaction:', error);
                    return; // a partial reaction cannot be processed
                }
            }
            ...
            for (const [commandId, command] of Object.entries(guildCommands)) {
                ...
                // Check if the reaction action matches (add/remove/both)
                if (command.trigger.action && command.trigger.action !== 'both' && command.trigger.action !== action) continue;

                // Check for a channel restriction
                if (command.trigger.channel && command.trigger.channel !== reaction.message.channel.id) continue;

                // More robust emoji matching
                const triggerEmoji = command.trigger.emoji;
                const reactionEmoji = reaction.emoji;

                const emojiMatch = (
                    triggerEmoji === reactionEmoji.name ||         // For standard emojis (e.g., '🙏')
                    triggerEmoji === reactionEmoji.id   ||         // For custom emoji IDs (e.g., '1234567890')
                    triggerEmoji === reactionEmoji.toString()      // For full custom emoji format (e.g., '<:name:id>')
                );
        ...
        for (const action of response.actions) {
            switch (action.type) {
                ...
                case 'message':
                    ...
                    if (action.ephemeral && context.message) {
                        await context.message.channel.send({ content: `${context.user.toString()}, ${content}` }).then(msg => setTimeout(() => msg.delete(), 5000));
                    } else {
