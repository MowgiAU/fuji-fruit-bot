...
class ReputationPlugin {
    ...
    async sendPublicReputationAnnouncement(guildId, fromUserId, toUserId, category, amount, reason, fallbackChannelId) {
        try {
            ...
            const targetChannelId = guildSettings.announcementChannel || fallbackChannelId;
            ...
            const fromUserMention = guildSettings.anonymousRep ? 'An anonymous user' : `<@${fromUserId}>`;
	   ...
	   return {
				   ...
				   <div class="settings-section">
					   ...
					   <div id="repSettingsContainer" style="display: none;">
						   ...
						   <div class="form-group">
							...
							<small>Channel to log detailed reputation events for moderation</small>
						</div>
						<div class="form-group">
							<label for="announcementChannelSelect">Announcement Channel</label>
							<select id="announcementChannelSelect">
								<option value="">Use current channel</option>
							</select>
							<small>Channel where public "Reputation Given!" messages will be posted</small>
						</div>
						   <div class="form-group">
							<label>
		   ...
		   script: `(function() {
			   ...
			   function populateSettings(settings) {
					...
					// Set channel values after channels are loaded
					console.log("🏆 Setting channel values:", {
						logChannel: settings.logChannel,
						announcementChannel: settings.announcementChannel
					});
					
					// Use a longer timeout to ensure channels are loaded
					setTimeout(() => {
						console.log("🏆 Attempting to set channel dropdown values...");
						
						if (settings.logChannel) {
							const logOption = logChannelSelect.querySelector('option[value="' + settings.logChannel + '"]');
							console.log("🏆 Log channel option found:", !!logOption, "for value:", settings.logChannel);
							if (logOption) {
								logChannelSelect.value = settings.logChannel;
								console.log("🏆 Set log channel to:", settings.logChannel);
							} else {
								console.warn("🏆 Log channel option not found for value:", settings.logChannel);
							}
						}
						
						if (settings.announcementChannel) {
							const announceOption = announcementChannelSelect.querySelector('option[value="' + settings.announcementChannel + '"]');
							console.log("🏆 Announcement channel option found:", !!announceOption, "for value:", settings.announcementChannel);
							if (announceOption) {
								announcementChannelSelect.value = settings.announcementChannel;
								console.log("🏆 Set announcement channel to:", settings.announcementChannel);
							} else {
								console.warn("🏆 Announcement channel option not found for value:", settings.announcementChannel);
							}
						}
						
						console.log("🏆 Final dropdown values - Log:", logChannelSelect.value, "Announcement:", announcementChannelSelect.value);
					}, 600); // Increased timeout to 600ms
				}
				   ...
				   saveRepSettings.addEventListener("click", async () => {
					   ...
					   try {
						   const settings = {
							   ...
							   announcementChannel: announcementChannelSelect.value || null,
